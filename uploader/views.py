...
def file_list(request):
    ...
    cloudinary_urls = [
        file["secure_url"] for file in cloudinary_files if "secure_url" in file
    ]

    # Створити об'єкти File для файлів з Cloudinary
    cloudinary_files_objects = [
        File(file=cloudinary_url) for cloudinary_url in cloudinary_urls
    ]
    ...
    images = [
        ...
        if file.file.name.lower().endswith((".jpg", ".png", ".gif", ".jpeg"))
    ]
    documents = [
        file
        for file in all_files
        if file.file.name.lower().endswith((".pdf", ".doc", ".docx", ".txt"))
    ]
    videos = [
        file
        for file in all_files
        if file.file.name.lower().endswith((".mp4", ".avi", ".mkv"))
    ]
...
def organize_files(request):
    ...
    for file in files:
        ...
        destination_path = os.path.join(
            settings.MEDIA_ROOT, f"user_uploads_{extension}", file.file.name
        )
